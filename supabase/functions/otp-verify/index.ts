...
Deno.serve(async (req) => {
  try {
    ...
    if (claim.outcome === "consumed") {
      return errorJson(409, "challenge_already_used");
    }
    ...
    if (claim.outcome === "max_attempts") {
      return errorJson(429, "max_attempts_reached");
    }
    if (claim.outcome !== "ready" || !claim.salt || !claim.code_hash) {
      return internalError(
        "otp-verify",
        new Error(`unexpected claim outcome: ${claim.outcome}`),
      );
    }
    ...
    const { data: marked, error: verifyError } = await db.rpc(
      "mark_otp_verified",
      {
        p_challenge_id: body.challengeId,
        p_user_id: caller.userId,
